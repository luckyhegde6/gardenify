...
export const apiClient = {
  ...
  async getSpeciesByName(scientificName: string): Promise<SpeciesListItem> {
  ...
  async login(
    email: string,
    password: string,
  ): Promise<import("@/lib/types").LoginResponse> {
    const response = await fetch(`${API_URL}/auth/login`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, password }),
    });
    return handleResponse<import("@/lib/types").LoginResponse>(response);
  },
  ...
  async adminGetUsers(
    ...
    return handleResponse<import("@/lib/types").AdminUserListResponse>(
      response,
    );
  },
  ...
  async adminResetPassword(token: string, userId: string): Promise<void> {
    const response = await fetch(
