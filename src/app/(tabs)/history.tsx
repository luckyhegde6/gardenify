...
export default function HistoryScreen() {
  ...
  const renderItem = ({ item }: { item: IdentificationRecord }) => {
    let result: {
      results?: {
        score: number;
        species: { scientific_name: string; common_names: string[] };
      }[];
    } | null = null;
    try {
    ...
    return (
      <PlantCard
        ...
        commonNames={
          commonNames.length > 0 ? commonNames : [item.species_common_names]
        }
        confidence={score}
        imageUrl={imageUrl}
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.header}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          {identifications.length} identification
          {identifications.length !== 1 ? "s" : ""}
        </Text>
      </View>
